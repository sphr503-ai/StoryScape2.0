    ...
    <div className={`h-screen bg-gradient-to-b ${config.genre === Genre.FANTASY ? 'from-emerald-950/40' : config.genre === Genre.SCIFI ? 'from-indigo-950/40' : 'from-slate-900'} to-black flex flex-col p-4 md:p-8 transition-colors duration-1000 overflow-hidden relative`}>
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 z-10 shrink-0">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <button onClick={handleDownloadSession} disabled={isDownloading} className="w-12 h-12 rounded-full glass border border-white/5 flex items-center justify-center hover:bg-white/10 transition-all">
          ...
          <button onClick={() => { setIsSummarizing(true); StoryScapeService.generateSummary(config.genre, transcriptions).then(s => { setSummary(s); setIsSummarizing(false); }); }} className="px-6 py-2.5 rounded-full bg-white text-black font-black text-xs uppercase tracking-widest">Finish</button>
          <button onClick={onExit} className="w-10 h-10 rounded-full bg-red-500/20 text-red-400 border border-red-500/10 flex items-center justify-center transition-all"><i className="fas fa-stop"></i></button>
        </div>
      </header>

      <main className="flex-1 flex flex-col max-w-5xl mx-auto w-full glass rounded-[2.5rem] overflow-hidden shadow-2xl relative border-white/10 z-10 bg-black/20 min-h-0">
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 md:p-12 space-y-10 scroll-smooth custom-scrollbar relative">
          ...
          {transcriptions.map((t, i) => (
            <div key={i} className="flex justify-start animate-in fade-in slide-in-from-bottom-2"><div className="max-w-[90%] p-6 rounded-[2rem] bg-black/30 border border-white/5 rounded-tl-none"><p className="text-[10px] opacity-40 mb-2 uppercase tracking-widest font-black">The Narrator</p><p className="text-lg md:text-xl leading-relaxed font-light">{t.text}</p></div></div>
          ))}
          {currentModelText && (
            <div className="flex justify-start"><div className="max-w-[90%] p-6 rounded-[2rem] bg-black/30 rounded-tl-none animate-pulse"><p className="text-lg md:text-xl leading-relaxed italic opacity-70">{currentModelText}</p></div></div>
          )}
        </div>
        <div className="p-6 md:p-8 glass border-t border-white/5 flex flex-col gap-4 bg-black/20 shrink-0">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <button onClick={togglePause} className={`w-12 h-12 rounded-full flex items-center justify-center transition-all ${isPaused ? 'bg-green-500 text-white' : 'glass border-white/10 hover:bg-white/10'}`}><i className={`fas ${isPaused ? 'fa-play' : 'fa-pause'}`}></i></button>
          </div>
